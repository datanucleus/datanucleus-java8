...
/**
 * Method for evaluating {dateExpr}.getMonthValue() using SQLite.
 * Returns a NumericExpression that equates to <pre>(strftime("%m", dateExpr))</pre>
 */
public class LocalDateGetMonthValue4Method extends AbstractSQLMethod
{
    /* (non-Javadoc)
    ...
    public SQLExpression getExpression(SQLExpression expr, List args)
    {
        ...
        {
            throw new NucleusException(Localiser.msg("060001", "getMonthValue()", expr));
        }

        RDBMSStoreManager storeMgr = stmt.getRDBMSManager();
        JavaTypeMapping mapping = storeMgr.getMappingManager().getMapping(String.class);
        SQLExpression month = exprFactory.newLiteral(stmt, mapping, "%m");
